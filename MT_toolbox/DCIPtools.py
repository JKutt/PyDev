...
class Jpatch:
    ...
    def getVpDivInErrors(self):
        ...
        for k in range(num_rdg):
            ...
            for j in range(num_dipole):
                if self.readings[k].Vdp[j].flagRho == "Accept":
                    err = (self.readings[k].Vdp[i].Vp_err + self.readings[k].Idp.In_err) / 100.
            ...
            if local:
                if direction is not None:
                    for k in range(num_rdg):
                        ...
                        for j in range(num_dipole):
                            if self.readings[k].Vdp[j].flagRho == "Accept":
                                sep = np.abs(self.readings[k].Vdp[j].getAseperationLocal(self, direction=direction))
                                a_list.append(sep)
                else:
                ...
                for k in range(num_rdg):
                    ...
                    for j in range(num_dipole):
                        if self.readings[k].Vdp[j].flagRho == "Accept":
                            sep = np.abs(self.readings[k].Vdp[j].getAseperation(self))
                            a_list.append(sep)
