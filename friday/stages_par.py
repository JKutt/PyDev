...
def get_content_folder(project):
    ...
    nodeList = datlib.list_nodes(project.path + '\\DATA\\')
    ...
    for node in nodeList:
        ...
        if node in nodeId:
            ...
            if len(ind):
                ...
                list_files = []
        logFile.write("Checking node: " + node + '\n')
        datNode = datlib.get_list_file(project.path + '\\DATA', node)
        datlib.check_list_files(datNode, logFile)
    logFile.write("Done! You can now check the file " + project.path + "\\checkDataFolder.log")
    logFile.close()
...
def get_records(project):
    ...
    if len(glob.glob(project.path + '\\nodeLibrary.dat')):
        records = datlib.read_library_file(project.path + '\\nodeLibrary.dat')
    ...
    for i in range(len(project.list_nodes)):
        ...
        r.append(pool.apply_async(datlib.read_node_library, (project.path, project.list_nodes[i], records, i)))
    pool.close()
    pool.join()
...
def coherency_analysis(project, injection, nodes):
    ...
    for i in range(len(nodes)):
        if injection.list_type[i] == 'A':       # Only getting nodes power
            signal.get_coherency(nodes[i], nodes, periods)
    """
    for ind in index_pool:
        tmp = r[ind].get()
