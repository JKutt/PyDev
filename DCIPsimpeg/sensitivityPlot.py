...
ya = 0.0
za = 400.0
xm_0 = np.arange(-1400.0, -800.0, 200)
xm_1 = np.arange(-900, -500, 100)
xm_2 = np.arange(-550, -150, 50)
xm_3 = np.arange(-100, 300, 100)
xm_4 = np.arange(400, 800, 200)
xm_5 = np.arange(1000, 1800, 400)
xm = np.hstack((xm_0, xm_1, xm_2, xm_3, xm_4, xm_5))
...
for idx in range(len(xm)):
  if xm[idx] != xa and xn[idx] != xa:
    ...
    J1 += np.abs(1 / (4 * np.pi**2) * (arg1 - arg2 - arg3 + arg4))

ym_ = np.ones_like(xm) + zm
yn_ = np.ones_like(xn) + zn
ax = plt.subplot(5, 1, 1, aspect='equal')
ax.contourf(x, z, np.log(J1), cmap='viridis', vmin=-32, vmax=-20)
# pcolorOpts = {}
...
ax.set_title("expanding dipoles (a=50,100,200,400) Forward and Reverse Dipoles")
# plt.show()
...
for idx in range(len(xm)):
  if xm[idx] != xa and xn[idx] != xa:
    ...
    arg4 = (((x - xb) * (x - xn[idx]) + (y - yb) * (y - yn) +
            ...
            (((x - xn[idx])**2 + (y - yn)**2 + (z - zn)**2)**-1.5))
    # evaluate sensitivity equation
    J2 += np.abs(1 / (4 * np.pi**2) * (arg1 - arg2 - arg3 + arg4))

ym_ = np.ones_like(xm) + zm
yn_ = np.ones_like(xn) + zn
ax2 = plt.subplot(5, 1, 2, aspect='equal')
ax2.contourf(x, z, np.log(J2), cmap='viridis', vmin=-32, vmax=-20)
tx1 = [xa, za]
ax2.plot(xm, ym_, 'k*')
ax2.plot(xn, yn_, 'k*')
...
ax2.set_title("a=50 Forward and Reverse Dipoles")
...
for idx in range(len(xm)):
  if xm[idx] != xa and xn[idx] != xa:
    ...
    arg4 = (((x - xb) * (x - xn[idx]) + (y - yb) * (y - yn) +
            ...
            (((x - xn[idx])**2 + (y - yn)**2 + (z - zn)**2)**-1.5))
    # evaluate sensitivity equation
    J += np.abs(1 / (4 * np.pi**2) * (arg1 - arg2 - arg3 + arg4))

ym_ = np.ones_like(xm) + zm
yn_ = np.ones_like(xn) + zn
ax3 = plt.subplot(5, 1, 3, aspect='equal')
ax3.contourf(x, z, np.log(J), cmap='viridis', vmin=-32, vmax=-20)
tx1 = [xa, za]
ax3.plot(xm, ym_, 'k*')
ax3.plot(xn, yn_, 'k*')
...
ax4.contourf(x, z, np.log(J), cmap='viridis', vmin=-32, vmax=-20)
tx1 = [xa, za]
ax4.plot(xm, ym_, 'k*')
ax4.plot(xn, yn_, 'k*')
...
ax5.contourf(x, z, np.log(J), cmap='viridis', vmin=-32, vmax=-20)
tx1 = [xa, za]
# ax4.plot(xm, ym_, 'k*')
# ax4.plot(xn, yn_, 'k*')
...
# diff ===========================================================
plt.contourf(x, z, np.log(np.abs(J2 - J1)), cmap='viridis')
plt.colorbar()
