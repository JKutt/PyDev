...
class decayKernal(baseKernel):
    ...
    window_weight = properties.Integer(
        "Attenuation factor",
        default=0.,
    )
    ...
    def __init__(self, num_windows=None, **kwargs):
    ...
    def _transform(self, stack):
        ...
        if isinstance(stack, np.ndarray):
            ...
            timebase = np.round(
                starts[self.num_windows - 1] / 1000.)
...
def getColeCole(mx_decay,
    ...
    for i in range(c.size):
        for j in range(tau.size):
            ...
            err = np.sqrt(serr / (window_weights.size - 1))
            if err < minErr:
                c_idx = i
                tau_idx = j
                minErr = (err)
...
class Jpatch:
    ...
    def createDcSurvey2D(self, data_type):
            """
            ...
            for k in range(num_rdg):
                ...
                rxx = []
                tx = np.array([self.readings[k].Idp.Tx1,
                              self.readings[k].Idp.Tx1Elev - d_off,
                              self.readings[k].Idp.Tx1 - 400.0,
                              self.readings[k].Idp.Tx2Elev - d_off])
                txx.append([self.readings[k].Idp.Tx1,
                           self.readings[k].Idp.Tx1North])
                if data_type == "DC":
                    for i in range(num_dipole):
                        try:
                            rho.append(self.readings[k].Vdp[i].Rho)
                            if self.readings[k].Vdp[i].flagRho != "Reject":
                                # print(self.readings[k].Vdp[i].Rho)
                                if self.readings[k].Vdp[i].Rho < 3000.:
                                    print("putting array")
                                    rxx.append([self.readings[k].Vdp[i].Rx1,
                                               self.readings[k].Vdp[i].Rx1Elev - d_off,
                                               self.readings[k].Vdp[i].Rx2,
                                               self.readings[k].Vdp[i].Rx2Elev - d_off])
                                    ...
                                    d_weights.append((self.readings[k].Vdp[i].Vp /
                                                     ...
                                                     (self.readings[k].Vdp[i].Vp_err /
                                                      100.0))
                                    zpp.append(self.readings[k].Vdp[i].Rx1Elev)
                                    zpp.append(self.readings[k].Vdp[i].Rx2Elev)
                    ...
                    for i in range(num_dipole):
                        try:
                            if self.readings[k].Vdp[i].flagMx != "Reject":
                                # print(self.readings[k].Vdp[i].Mx / 1000.)
                                if self.readings[k].Vdp[i].Mx < 35.:
                                    rxx.append([self.readings[k].Vdp[i].Rx1,
                                               self.readings[k].Vdp[i].Rx1Elev - d_off,
                                               self.readings[k].Vdp[i].Rx2,
                                               self.readings[k].Vdp[i].Rx2Elev - d_off])
                                    ...
                                    data.append((self.readings[k].Vdp[i].Mx *
                                                (self.readings[k].Vdp[i].Vp / 1000.)) / 1000.)
                                    d_weights.append(((self.readings[k].Vdp[i].Mx / 1000.) *
                                                     (5. /
                                                      100.0)))
                ...
                # print(rx)
                try:
                    rxClass = DC.Rx.Dipole(rx[:, :2], rx[:, 2:])  # create dipole list
                    srcClass = DC.Src.Dipole([rxClass], tx[:2], tx[2:])
                    srcLists.append(srcClass)
                except:
                    print("could not add dipole")

            if data_type == "DC":
                survey = DC.Survey(srcLists)          # creates the survey
            elif data_type == "IP":
                survey = IP.Survey(srcLists)          # creates the survey
            survey.dobs = np.asarray(data)                 # assigns data
            # print(np.max(np.asarray(zpp)))
            survey.std = np.abs(np.asarray(d_weights))     # assign data weights
            survey.eps = np.ones_like(data) * (10**-3.2)
            # print(np.max(np.asarray(zpp)))
    ...
    def createDcSurvey(self, data_type):
            ...
            for k in range(num_rdg):
                ...
                txx.append([self.readings[k].Idp.Tx1East,
                           self.readings[k].Idp.Tx1North])
                if data_type == "DC":
                    for i in range(num_dipole):
                        try:
                            if self.readings[k].Vdp[i].flagRho != "Reject":
                                if (self.readings[k].Vdp[i].Vp / self.readings[k].Idp.In) < 0.6:
                                    rxx.append([self.readings[k].Vdp[i].Rx1East,
                    ...
                    for i in range(num_dipole):
                        try:
                            if self.readings[k].Vdp[i].flagMx != "Reject":
                                if self.readings[k].Vdp[i].Mx / 1000. < 0.06:
                                    rxx.append([self.readings[k].Vdp[i].Rx1East,
                                               self.readings[k].Vdp[i].Rx1North,
                                               self.readings[k].Vdp[i].Rx1Elev,
                                               self.readings[k].Vdp[i].Rx2East,
                ...
                rx = np.asarray(rxx)
                rxClass = DC.Rx.Dipole(rx[:, :3], rx[:, 3:])  # create dipole list
                srcClass = DC.Src.Dipole([rxClass], tx[:3], tx[3:])
                srcLists.append(srcClass)
...
class Jreadtxtline:
    ...
    def __init__(self, hdrLine, dataLine):
        ...
        for n in range(len(hdrLine)):
            if hdrLine[n].find("M") == 0 and hdrLine[n].find("Mx") != 0:
                try:
                    self.Vs.append(float(dataLine[n]))
                except:
                    self.Vs.append(-999.9)
            else:
